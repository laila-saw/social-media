...
function Feed({ userId }) {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      (userId ?
        axiosInstance.get('/posts/profile/' + userId) :
        axiosInstance.get('posts/timeline/' +user._id))
        .then(res => setPosts(
          res.data.sort((p1,p2)=>{
...
function Post({ post }) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      axiosInstance.get(`/users?userId=${post.userId}`)
 ...
 async function editPost(){
    try{
      await axiosInstance.put("/posts/"+post._id,{userId:currentUser._id,desc:laila.current.value});
      window.location.reload();
    }catch(error){
      console.log(error);
    }
...
function Rightbar({ user }) {
  ...
  useEffect(()=>{
    const getFollowings=async()=>{
      try{
      const followings = await axiosInstance.get("/users/followings/"+user?._id);
...
function Profile() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      axiosInstance.get(`/users?userId=${userId}`)
        .then(res => setUser(res.data))
        .catch(err => {
          console.log(err)
        })
